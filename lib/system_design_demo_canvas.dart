// Generic Demo Canvas Screen for System Design Visualizations
// This screen shows only the diagram and explanation (no editing controls)
// Can be used for any system design type (URL Shortener, Gaming Leaderboard, etc.)
...
/// Generic Demo Canvas Screen for viewing any system design
class SystemDesignDemoCanvas extends StatefulWidget {
  final String designName;
  final Map<String, dynamic> designData;
  final Color? themeColor;

  const SystemDesignDemoCanvas({
    super.key,
  ...
  @override
  State<SystemDesignDemoCanvas> createState() => _SystemDesignDemoCanvasState();
}

class _SystemDesignDemoCanvasState extends State<SystemDesignDemoCanvas> {
  List<DemoDroppedIcon> icons = [];
  List<DemoDrawnLine> lines = [];
  late TransformationController _transformationController;

  Color get themeColor => widget.themeColor ?? Colors.purpleAccent;
  ...
  void _fitDesignToScreen() {
    ...
    for (final icon in icons) {
      ...
      maxX = math.max(maxX, icon.position.dx + 70);
      maxY = math.max(maxY, icon.position.dy + 80);
    }

    final screenSize = MediaQuery.of(context).size;
    final canvasWidth = screenSize.width;
    final canvasHeight = screenSize.height - 150;

    final designWidth = maxX - minX + 100;
    final designHeight = maxY - minY + 100;
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                Text(
                  '• ',
                  style: GoogleFonts.saira(fontSize: 14, color: themeColor),
                ),
                Expanded(
                  child: Text(
                    line.substring(2),
